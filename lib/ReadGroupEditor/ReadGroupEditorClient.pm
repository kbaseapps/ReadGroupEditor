package ReadGroupEditor::ReadGroupEditorClient;
...
ReadGroupEditor::ReadGroupEditorClient
...
sub new
{
    ...
    my $self = {
	client => ReadGroupEditor::ReadGroupEditorClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
save_read_group_params is a reference to a hash where the following keys are defined:
	...
	output_readgroup_name has a value which is a string
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "ReadGroupEditor.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for ReadGroupEditor::ReadGroupEditorClient\n";
    }
    if ($sMajor == 0) {
        warn "ReadGroupEditor::ReadGroupEditorClient version is $svr_version. API subject to change.\n";
    }
}
...
package ReadGroupEditor::ReadGroupEditorClient::RpcClient;
